...
const modules: Record<string, ModuleDef> = {
  ...
  send_sticker: method(
    "send_sticker",
    "Send a sticker",
    z.object({ chatId, sticker: z.string() }),
    "sendSticker",
    (p) => ({ chat_id: p.chatId, sticker: p.sticker }),
  ),
  send_dice: method(
    "send_dice",
    "Send a dice",
    z.object({ chatId, emoji: z.string().optional() }),
    "sendDice",
    (p) => ({ chat_id: p.chatId, emoji: p.emoji }),
  ),
  send_chat_action: method(
    "send_chat_action",
    "Send a chat action",
    z.object({ chatId, action: z.string() }),
    "sendChatAction",
    (p) => ({ chat_id: p.chatId, action: p.action }),
  ),
  edit_message_caption: method(
    "edit_message_caption",
    "Edit a message caption",
    z.object({ chatId, messageId: z.number(), caption: z.string(), parseMode }),
    "editMessageCaption",
    (p) => ({ chat_id: p.chatId, message_id: p.messageId, caption: p.caption, parse_mode: p.parseMode }),
  ),
  edit_message_reply_markup: method(
    "edit_message_reply_markup",
    "Edit a message reply markup",
    z.object({ chatId, messageId: z.number(), replyMarkup: z.any() }),
    "editMessageReplyMarkup",
    (p) => ({ chat_id: p.chatId, message_id: p.messageId, reply_markup: p.replyMarkup }),
  ),
  stop_poll: method(
    "stop_poll",
    "Stop a poll",
  ...
  set_message_reaction: method(
    "set_message_reaction",
    "Set a message reaction",
    z.object({ chatId, messageId: z.number(), reaction: z.array(z.any()).optional() }),
    "setMessageReaction",
    (p) => ({ chat_id: p.chatId, message_id: p.messageId, reaction: p.reaction }),
  ),
  // ---- chat administration ----
  ban_chat_member: method(
    "ban_chat_member",
    "Ban a chat member",
    z.object({ chatId, userId: z.number(), untilDate: z.number().optional() }),
    "banChatMember",
    (p) => ({ chat_id: p.chatId, user_id: p.userId, until_date: p.untilDate }),
  ),
  unban_chat_member: method(
    "unban_chat_member",
    "Unban a chat member",
    z.object({ chatId, userId: z.number(), onlyIfBanned: z.boolean().optional() }),
    "unbanChatMember",
    (p) => ({ chat_id: p.chatId, user_id: p.userId, only_if_banned: p.onlyIfBanned }),
  ),
  restrict_chat_member: method(
    "restrict_chat_member",
    "Restrict a chat member",
    z.object({ chatId, userId: z.number(), permissions: z.any(), untilDate: z.number().optional() }),
    "restrictChatMember",
    (p) => ({ chat_id: p.chatId, user_id: p.userId, permissions: p.permissions, until_date: p.untilDate }),
  ),
  promote_chat_member: method(
    "promote_chat_member",
    "Promote a chat member",
    z.object({ chatId, userId: z.number(), canManageChat: z.boolean().optional(), canDeleteMessages: z.boolean().optional(), canRestrictMembers: z.boolean().optional(), canPinMessages: z.boolean().optional(), canInviteUsers: z.boolean().optional() }),
    "promoteChatMember",
    (p) => ({ chat_id: p.chatId, user_id: p.userId, can_manage_chat: p.canManageChat, can_delete_messages: p.canDeleteMessages, can_restrict_members: p.canRestrictMembers, can_pin_messages: p.canPinMessages, can_invite_users: p.canInviteUsers }),
  ),
  set_chat_title: method(
    "set_chat_title",
    "Set chat title",
    z.object({ chatId, title: z.string() }),
    "setChatTitle",
    (p) => ({ chat_id: p.chatId, title: p.title }),
  ),
  set_chat_description: method(
    "set_chat_description",
    "Set chat description",
    z.object({ chatId, description: z.string() }),
    "setChatDescription",
    (p) => ({ chat_id: p.chatId, description: p.description }),
  ),
  leave_chat: method(
    "leave_chat",
    "Leave a chat",
  ...
  // ---- more reads ----
  get_me: method("get_me", "Get bot info", z.object({}), "getMe", () => ({}), "search"),
  get_chat_administrators: method(
  ...
  get_user_profile_photos: method(
    "get_user_profile_photos",
    "Get user profile photos",
    z.object({ userId: z.number(), limit: z.number().optional() }),
    "getUserProfilePhotos",
    (p) => ({ user_id: p.userId, limit: p.limit }),
    "search",
  ),
};
